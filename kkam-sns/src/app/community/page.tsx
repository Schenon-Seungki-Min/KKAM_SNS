...
export default function CommunityPage() {
  ...
  const sourceCounts = {
    ...
    youtube: state.results.filter((r) => r.source === 'youtube').length,
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">
                키워드를 입력하면 네이버 블로그/카페/지식iN, 유튜브에서 검색합니다
              </p>
            </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
          ...
          <>
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-600">
                <span className="font-semibold">&quot;{state.keyword}&quot;</span> 검색 결과{' '}
                <span className="font-semibold">{state.totalCount}</span>건
              </p>
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                >
                  {tab.label} ({sourceCounts[tab.key as keyof typeof sourceCounts]})
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div>
                <WordCloud keywords={state.relatedKeywords} onKeywordClick={handleSearch} />
              </div>
            </div>
          </>
        )}
        ...
        {!state.loading && state.results.length === 0 && !state.error && (
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="text-lg">키워드를 입력하여 커뮤니티를 분석해보세요</p>
            <p className="text-sm mt-2">
              네이버 블로그, 카페, 지식iN, 유튜브에서 검색 결과를 가져옵니다
            </p>
          </div>
        )}
